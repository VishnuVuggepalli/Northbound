...
| Pica8 PicOS | `pica8` | ✓ | **real-device** (captured) | **read ✓ LIVE / write ✗ (wrong schema, confirmed)** | **PicOS-V 4.2.2** (qemu/KVM, NETCONF) | Obtained PicOS-V (public Drive link off pica8.com), booted, enabled NETCONF (`protocols netconf` in xorp pica.conf), and ran the real driver. **Confirmed the authored data model was fabricated**: real PicOS uses `<interface xmlns="http://pica8.com/xorplus/interface"><gigabit-ethernet>` — NOT Juniper-xnm `<interfaces><interface><name>`. **Rewrote the read path + fixed 2 real bugs** (port element type; `<disable>` is a boolean-VALUE leaf not a presence flag): `get_ports` now parses all 78 real ports w/ correct admin state, validated live. Real-device fixture captured. **Write path: schema wrong AND unvalidatable here.** (1) `_build_edit_config_xml` still emits Junos-xnm; real PicOS uses xorplus namespaces. (2) Can't validate/learn the real write schema on this image: PicOS-V's virtual switching dataplane (`pica_cardmgr`, no real ASIC) never reaches ready, so config-mode `set`/`commit` won't even stage (`show | compare` empty; every cmd returns "PICOS is starting…") — reads work, L2 writes are blocked. NOT blind-rewritten (no real VLAN-config example to copy = the circular-fixture trap again). Needs real PicOS hardware or a licensed PicOS-V where the dataplane initializes. |
