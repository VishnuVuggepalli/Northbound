...
| Pica8 PicOS | `pica8` | ✓ | **real-device** (captured) | **read ✓ LIVE / write ✗ (wrong schema, confirmed)** | **PicOS-V 4.2.2** (qemu/KVM, NETCONF) | Obtained PicOS-V (public Drive link off pica8.com), booted, enabled NETCONF (`protocols netconf` in xorp pica.conf), and ran the real driver. **Confirmed the authored data model was fabricated**: real PicOS uses `<interface xmlns="http://pica8.com/xorplus/interface"><gigabit-ethernet>` — NOT Juniper-xnm `<interfaces><interface><name>`. **Rewrote the read path + fixed 2 real bugs** (port element type; `<disable>` is a boolean-VALUE leaf not a presence flag): `get_ports` now parses all 78 real ports w/ correct admin state, validated live. Real-device fixture captured. **Write path still WRONG** (`_build_edit_config_xml` still emits Junos-xnm; real PicOS edit-config uses xorplus namespaces, and this OVS-mode trial has no Junos `port-mode`/`family` model at all) — rewrite tracked separately. |
